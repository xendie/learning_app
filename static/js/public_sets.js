    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(response.message) {
                ...
                let practiceButton = $('<a class="dynamic-practice-button" href="/practice/' + dataId + '"><button>Practice</button></a>');
                $('.options-bar-buttons').append(practiceButton);
...
function backToPracticeSets(){
    ...
    $('.dynamic-practice-button').remove();
}
...
function getPracticeSets(page = '1'){
    $.ajax({
        url: siteUrl + '/get_public_practice_sets/' + page,
        method: 'GET',
        dataType: 'json',
        xhrFields: {
            withCredentials: true
        ...
        success: function(response){
            ...
            if (response.message) {
                ...
                $.each(response.message, function(index, practiceSet) {
                    table += '<tr data-id="' + practiceSet.id + '" data-name="' + practiceSet.set_name + '" user-id="' + practiceSet.user_id +'">' +
                            '<td class="index-cell">' + (((page - 1) * 20) + index + 1) + '</td>' +
                            '<td>' + practiceSet.set_name + '</td>' +
                            '<td class="options-cell">' +
                                '<button onclick="setView(this)" class="btn-view">View</button>' +
                                '<a href="practice/' + practiceSet.id + '"><button onclick="setPractice(this)" class="btn-practice">Practice</button></a>' +
                            '</td>' +
                        '</tr>';
                });
